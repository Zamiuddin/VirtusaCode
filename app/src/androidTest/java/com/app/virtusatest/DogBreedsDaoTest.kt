...
import com.app.virtusatest.data.local.persistence.DogBreedsDatabase
...
class DogBreedsDaoTest {
    ...
    fun setupDatabase() {
        database = Room.inMemoryDatabaseBuilder(
            ApplicationProvider.getApplicationContext(), DogBreedsDatabase::class.java
    ...
    fun insertAllBreeds_returnsTrue() = runBlocking {
        val breed1 = DogBreed(
            name = "african", subBreeds = "", isFavourite = false, imageUrl = "dummy_url"
        )
        val breed2 = DogBreed(
            name = "australian", subBreeds = "", isFavourite = false, imageUrl = "dummy_url"
        )
    ...
    fun updateDogBreeds_returnsTrue() = runBlocking {
        ...
        val updatedBreed = DogBreed(
            name = "african", subBreeds = "", isFavourite = true, imageUrl = "dummy_url"
        )
